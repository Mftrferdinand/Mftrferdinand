...
# Karakter untuk background matrix rain
...
# - DECRYPTING / DECODE (karakter scramble mengunci per huruf): 14 frame
# - SOLID HOLD (semua teks putih tajam, matrix rain tetap mengalir): 16 frame
# - DISSOLVE EXIT: 6 frame
...
print(f"Generating {TOTAL_FRAMES} matrix frames on black background...")
...
for gi in range(TOTAL_FRAMES):
    ...
    # 1. Base canvas: Hitam pekat solid
    canvas = Image.new("RGBA", (bw, bh), BLACK)

    # 2. Render Matrix Rain hacker effect di background (monokrom / subtle code rain)
    rain_draw = ImageDraw.Draw(canvas)
    for col in rain_columns:
        head_y = (col["y"] + gi * col["speed"]) % (bh + col["length"] * row_h) - (
            col["length"] * row_h
        )
        for ci in range(col["length"]):
            ...
            if -row_h <= char_y < bh + row_h:
                ...
                if ci == 0:
                    # Head leader: Putih tajam terang
                    c_color = (255, 255, 255, 200)
                elif ci < 3:
                    # Upper trail: Abu-abu terang
                    c_color = (190, 190, 190, 140)
                else:
                    # Lower trail: Abu-abu redup
                    fade = 1.0 - (ci / col["length"])
                    alpha = int(90 * fade)
                    c_color = (130, 130, 130, alpha)
    ...
    # 3. Main Text (SEMUA TEKS PUTIH MURNI DENGAN EFEK DECRYPT HACKER)
    ...
    for idx, cinfo in enumerate(cp["chars"]):
        if idx < locked_count:
            # Karakter yang sudah ter-decode: Putih solid
            char_to_draw = cinfo["char"]
            char_color = (255, 255, 255, text_alpha)
        else:
            # Karakter yang masih mengacak / decrypting: Monospace random putih/abu-abu
            char_to_draw = random.choice(SCRAMBLE_CHARS) if text_alpha > 40 else ""
            char_color = (200, 200, 200, int(text_alpha * 0.75))
...
print("Saved matrix banner cleanly to", out_path)
